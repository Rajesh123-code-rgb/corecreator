...
export function SSOButton({ provider, callbackUrl = "/", className = "" }: SSOButtonProps) {
    ...
    return (
        <button
            ...
            className={`w-full flex items-center justify-center gap-3 px-4 py-3 rounded-lg border border-[var(--border)] font-medium text-sm transition-all ${config.bgColor} ${config.textColor} ${className}`}
        >
            {config.icon}
