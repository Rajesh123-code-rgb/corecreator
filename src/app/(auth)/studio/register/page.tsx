...
const registerSchema = z
    .object({
        name: z.string().min(2, "Studio/Artist Name must be at least 2 characters"),
...
export default function StudioRegisterPage() {
    ...
    const onSubmit = async (data: RegisterFormData) => {
        ...
        try {
            const response = await fetch("/api/auth/register", {
                ...
                body: JSON.stringify({ name: data.name, email: data.email, password: data.password, role: "studio" }),
            });
            const result = await response.json();
            if (!response.ok) throw new Error(result.message);
            setSuccess(true);
            setTimeout(() => signIn("credentials", { email: data.email, password: data.password, callbackUrl: "/studio/dashboard" }), 1500);
        } catch (err) {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center p-8">
                <div className="text-center">
                    ...
                    <h1 className="text-2xl font-bold mb-2">Welcome to Core Creator Studio!</h1>
                    <p className="text-[var(--muted-foreground)]">Setting up your studio...</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen flex">
            {/* Left - Hero */}
            <div className="hidden lg:flex flex-1 items-center justify-center p-12 bg-neutral-900 relative overflow-hidden">
                ...
                <div className="relative w-full h-full max-w-2xl text-center flex flex-col items-center justify-center z-10">
                    ...
                    <ul className="mt-8 text-left space-y-4 text-white/80">
                        <li className="flex items-center gap-3">
                            ...
                            <span>Create and sell unlimited courses</span>
                        </li>
                        <li className="flex items-center gap-3">
                            <CheckCircle className="w-5 h-5 text-[var(--secondary-500)]" />
                            <span>List your original artworks for sale</span>
                        </li>
                        <li className="flex items-center gap-3">
                            <CheckCircle className="w-5 h-5 text-[var(--secondary-500)]" />
            ...
            <div className="flex-1 flex items-center justify-center p-8 overflow-y-auto">
                <div className="w-full max-w-md">
                    ...
                    <div className="mb-6">
                        <span className="inline-block px-3 py-1 rounded-full bg-[var(--secondary-100)] text-[var(--secondary-700)] text-xs font-semibold mb-2">Creator Account</span>
                        <h1 className="text-2xl font-bold">Create Studio Account</h1>
                        <p className="text-[var(--muted-foreground)]">Start teaching and selling today</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                        <Input label="Studio / Artist Name" placeholder="E.g. Da Vinci Studios" leftIcon={<User className="w-5 h-5" />} error={errors.name?.message} {...register("name")} />
                        <Input label="Email" type="email" placeholder="studio@email.com" leftIcon={<Mail className="w-5 h-5" />} error={errors.email?.message} {...register("email")} />
                        ...
                        <div className="flex items-start gap-2">
                            ...
                            <label htmlFor="terms" className="text-sm text-[var(--muted-foreground)]">
                                I agree to the <Link href="/terms" className="text-[var(--secondary-500)] hover:underline">Terms</Link> applied for Creator Accounts.
                            </label>
                        </div>
